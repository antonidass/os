...
idescr struc
    ...
    sel     dw 0 ; Селектор  
    cntr    db 0 ; Счетчик.
    attr    db 0 ; Байты атрибутов (такой же как и в descr атрибут attr_1)
...
data32 segment para 'data'
    ...
    gdt_data4gb descr <0FFFFh,0,0,92h,0CFh,0> ; 65535 92h = 10010010 0CFh = 110011110    
    ...
    int_time idescr <0,code32s,0,10001110b,0> 
    int_key idescr <0,code32s,0,10001110b,0>
...
pm_start:
    ...
    new_int_key proc ;uses eax ebx edx
		push eax
		push ebx
		push edx
		push ds
...
; Заносим в дескрипторы прерываний (шлюзы);
    lea eax, es:except_1
    ...
    lea eax, es:new_int_time
    mov int_time.offs_l, ax
    shr eax, 16
    mov int_time.offs_h, ax

    lea eax, es:new_int_key
